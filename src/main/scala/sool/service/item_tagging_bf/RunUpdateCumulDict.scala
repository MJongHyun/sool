...
class RunUpdateCumulDict(spark: org.apache.spark.sql.SparkSession) {
  ...
  def getCumulNminfoDf(bfDictDf: DataFrame, fnlNminfoDf: DataFrame) = {
    ...
    val colsOrder2 = Seq("NAME", "ITEM_SZ", "AMT", "ITEM", "SAW").map(col)
  ...
  // 메인
  def runUpdateCumulDict(ethDt: String, flag: String): Unit = {
    ...
    val bfDictDf = spark.read.parquet(filePathCls.beforeDictDfPath)
    val fnlNminfoDf = spark.read.parquet(filePathCls.fnlNminfoDfPath)

    /* 누적 품목사전 업데이트 */
    val cumulNminfoDf = getCumulNminfoDf(bfDictDf, fnlNminfoDf)
    if (checkCumulNminfoDf(cumulNminfoDf) == false) return
    fileFuncCls.wParquet(cumulNminfoDf, filePathCls.cumulNminfoDfPath) // 누적 품목사전 업데이트(저장)

    /* 전월 누적 품목사전과 이번 달 누적 품목사전 비교 확인 */
    val cumulBf = if (fileFuncCls.checkS3FileExist(filePathClsBf1m.cumulNminfoDfNewPath)) {
      spark.read.parquet(filePathClsBf1m.cumulNminfoDfNewPath)  // 누적사전 변경으로 인해 경로 변경 시 사용됨
    } else {
      spark.read.parquet(filePathClsBf1m.cumulNminfoDfPath)
    }
    val cumulAf = spark.read.parquet(filePathCls.cumulNminfoDfPath)
    showInfoAboutCumulNminfoDf(cumulBf, cumulAf, ethDtBf1m, ethDt)  // 관련 정보 확인
  }
}
