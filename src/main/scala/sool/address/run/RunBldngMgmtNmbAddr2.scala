...
object RunBldngMgmtNmbAddr2 {
  ...
  val rfnAddrCls = new BldngMgmtNmbAddr2(spark)
  ...
  def main(args: Array[String]):Unit = {
    ...
    slack.sendMsg(flag, "원천주소 추출 및 원천주소별 건물관리번호 수집 시작(주소 우회)", fileName)
    logger.info(s"[file=RunBldngMgmtNmbAddr] [function=main] [status=start] [message=${ethDt} RunBldngMgmtNmbAddr 작업 시작]")
    ...
    logger.info(s"[file=RunBldngMgmtNmbAddr] [function=main] [status=end] [message=${ethDt} RunBldngMgmtNmbAddr 작업 종료]")
    slack.sendMsg(flag, "원천주소 추출 및 원천주소별 건물관리번호 수집 완료(주소 우회)", fileName)
  }
}